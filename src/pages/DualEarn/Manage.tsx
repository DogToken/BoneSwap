...
import { CardSection, DataCard, CardNoise, CardBGImage } from '../../components/dualEarn/styled'
import { ButtonPrimary, ButtonEmpty } from '../../components/Button'
import StakingModal from '../../components/dualEarn/StakingModal'
import { useDualStakingInfo } from '../../state/stake/hooks'
import UnstakingModal from '../../components/dualEarn/UnstakingModal'
import ClaimRewardModal from '../../components/dualEarn/ClaimRewardModal'
...
export default function Manage({
  ...
  const newStakingInfos = useDualStakingInfo(stakingTokenPair)
  const stakingInfos = newStakingInfos;
  ...
  const countUpAmountA = stakingInfo?.earnedAmountA?.toFixed(6) ?? '0'
  const countUpAmountPreviousA = usePrevious(countUpAmountA) ?? '0'

  const countUpAmountB = stakingInfo?.earnedAmountB?.toFixed(6) ?? '0'
  const countUpAmountPreviousB = usePrevious(countUpAmountB) ?? '0'
    ...
    <PageWrapper gap="lg" justify="center">
      ...
      <PositionInfo gap="lg" justify="center" dim={showAddLiquidityButton}>
        <BottomSection gap="lg" justify="center">
          ...
          <StyledBottomCard dim={stakingInfo?.stakedAmount?.equalTo(JSBI.BigInt(0))}>
            ...
            <AutoColumn gap="sm">
              <RowBetween>
                <div>
                  <TYPE.black>Your unclaimed {stakingInfo?.rewardTokenA?.symbol}</TYPE.black>
                </div>
              </RowBetween>
              <RowBetween style={{ alignItems: 'baseline' }}>
                <TYPE.largeHeader fontSize={36} fontWeight={600}>
                  <CountUp
                    ...
                    isCounting
                    decimalPlaces={4}
                    start={parseFloat(countUpAmountPreviousA)}
                    end={parseFloat(countUpAmountA)}
                    thousandsSeparator={','}
                    duration={1}
                  />
                </TYPE.largeHeader>
            ...
            <AutoColumn gap="sm">
              <RowBetween>
                <div>
                  <TYPE.black>Your unclaimed {stakingInfo?.rewardTokenB?.symbol}</TYPE.black>
                </div>
              </RowBetween>
              <RowBetween style={{ alignItems: 'baseline' }}>
                <TYPE.largeHeader fontSize={36} fontWeight={600}>
                  <CountUp
                    ...
                    isCounting
                    decimalPlaces={4}
                    start={parseFloat(countUpAmountPreviousB)}
                    end={parseFloat(countUpAmountB)}
                    thousandsSeparator={','}
                    duration={1}
                  />
                </TYPE.largeHeader>
            ...
            <AutoColumn gap="sm">
              <RowBetween>
                {stakingInfo?.earnedAmountB && JSBI.notEqual(BIG_INT_ZERO, stakingInfo?.earnedAmountB?.raw) && (
                  <ButtonEmpty
                    padding="8px"
                    borderRadius="8px"
                    width="fit-content"
        ...
        <TYPE.main style={{ textAlign: 'center' }} fontSize={14}>
          ...
          When you withdraw, the contract will automagically claim {stakingInfo?.rewardTokenA?.symbol} and {stakingInfo?.rewardTokenB?.symbol} on your behalf!
        </TYPE.main>
        ...
        {!showAddLiquidityButton && (
          <DataRow style={{ marginBottom: '1rem' }}>
            ...
            <ButtonPrimary padding="8px" borderRadius="8px" width="160px" onClick={handleDepositClick}>
              {stakingInfo?.stakedAmount?.greaterThan(JSBI.BigInt(0)) ? 'Deposit' : stakingInfo  && stakingInfo?.name !== '' ? 'Deposit ' + stakingInfo?.name + " Tokens": 'Deposit LP Tokens'}
            </ButtonPrimary>
            } 
